...
export default async function handler(req, res){
  if (req.method === 'GET'){
    const user = readAuth(req); if (!user) return res.status(401).json({ ok:false, error:'Unauthorized' });
  ...
  if (req.method === 'POST'){
    ...
    if (!p.rowCount) return res.status(400).json({ ok:false, error:'Invalid product' });
    const total = p.rows[0].price_cents * Number(qty||1);
    const o = await query('insert into orders (user_id, total_cents, status) values ($1,$2,$3) returning id', [user.sub, total, 'paid']);
    await query('insert into order_items (order_id, product_id, unit_price_cents, qty) values ($1,$2,$3,$4)', [o.rows[0].id, product_id, p.rows[0].price_cents, Number(qty||1)]);
    await query('insert into entitlements (user_id, product_id) values ($1,$2) on conflict do nothing', [user.sub, product_id]);
    return res.status(200).json({ ok:true, data:{ order_id: o.rows[0].id } });
  }
